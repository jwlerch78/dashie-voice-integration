...
| Bridge auth | `addon_bridge.py` (`X-Chickadee-Bridge-Secret` header; secret read from `<ha-config>/.chickadee/bridge_secret`, fallback glob `addon_configs/*chickadee/bridge_secret`) | add-on writes the secret to its homeassistant_config mount (`.chickadee/bridge_secret`) + addon_config at startup; auth ENFORCED from birth (401 on missing/bad header) | ⚠️ HA Core canNOT read /addon_configs on HAOS (verified 2026-07-25) — the HA-config-dir copy is the working channel. INTERIM (readable by other config-mounted add-ons); replace with Supervisor discovery. Pattern: Dashie 20260702_BRIDGE_AUTH_HARDENING |
