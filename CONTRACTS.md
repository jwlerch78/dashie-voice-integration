...
| Bridge auth | `addon_bridge.py` (`X-Chickadee-Bridge-Secret` header; secret read via glob `addon_configs/*chickadee/bridge_secret` — installed slug is prefixed: `local_chickadee` / `<repo-hash>_chickadee`) | add-on writes `bridge_secret` into its addon_config mount at startup; auth ENFORCED from birth (401 on missing/bad header) | Pattern: Dashie 20260702_BRIDGE_AUTH_HARDENING |
